...
public class CoinCollisionProcedure {
	public static void execute(LevelAccessor world, Entity entity, Entity sourceentity) {
		if (entity == null || sourceentity == null)
			return;
		double old_coin_count = 0;
		double new_coin_count = 0;
		old_coin_count = (sourceentity.getCapability(MarioCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new MarioCraftModVariables.PlayerVariables())).CoinNumber;
		if ((sourceentity.getCapability(MarioCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new MarioCraftModVariables.PlayerVariables())).Grabbed_Coin == false) {
			...
			{
				double _setval = (sourceentity.getCapability(MarioCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new MarioCraftModVariables.PlayerVariables())).CoinNumber + 1;
				sourceentity.getCapability(MarioCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
					capability.CoinNumber = _setval;
		...
		new_coin_count = (sourceentity.getCapability(MarioCraftModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new MarioCraftModVariables.PlayerVariables())).CoinNumber;
